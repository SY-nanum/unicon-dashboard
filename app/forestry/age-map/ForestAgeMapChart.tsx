...
/* Region centroid coordinates [lng, lat] */
        ...
        chart.setOption({
          ...
          tooltip: {
            ...
            formatter: (p: { data?: { label: string; value: [number, number, number] } }) =>
              p.data ? `<b>${p.data.label}</b><br/>${p.data.value[2].toFixed(2)} ${unit}` : '',
          },
          geo: {
            map: 'world',
            roam: true,
          ...
          series: [
            {
              ...
              itemStyle: { color: '#16a34a', opacity: 0.72, borderColor: '#064e3b', borderWidth: 1 },
              label: {
                show: true,
...
export function ForestAgeMapChart({ rows }: Props) {
  ...
  const gisData = toGisPoints(buildData(rows, selectedYear, gisScenario));
  const gisTitle = `산림 면적 분포 — ${selectedYear}년 ${gisScenario === 'Historical' ? '실적' : gisScenario} (Mha)`;
  ...
  const gisContent = (
    <>
      ...
      <GisWorldMap data={gisData} unit="Mha" title={gisTitle} />
      <div className="rounded-lg bg-green-50 p-3 text-xs text-green-800">
        <b>버블 크기</b> = 전체 산림 면적 합계 (Mha). 버블 위에 마우스를 올리면 권역별 수치를 확인할 수 있습니다.
