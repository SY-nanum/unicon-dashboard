...
export function ForestBiomassChart({ rows }: Props) {
  ...
  // Fixed max across all years & scenarios — bubble sizes stay comparable when changing years
  const globalMax = useMemo(() => {
    let mx = 1;
    for (const y of TARGET_YEARS) {
      for (const sc of ['BAU', 'NetZero', 'Historical'] as const) {
        for (const r of getRegionData(rows, y, sc)) mx = Math.max(mx, r.value);
      }
    }
    return mx;
  }, [rows]);
  ...
  const gisBubbleColor = SC_COLOR[gisSc]; // BAU=red, NetZero=green, Historical=gray
  ...
  const gisContent = (
    <>
      <div className="flex flex-wrap items-center gap-4">{yearSelector}{!isHistorical && modeSelector}</div>
      {isCompare
        ? <GisWorldMap compare bauData={bauGis} nzData={nzGis} fixedMax={globalMax} unit="Mt C" title={gisTitle} />
        : <GisWorldMap data={singleGis} singleColor={gisBubbleColor} fixedMax={globalMax} unit="Mt C" title={gisTitle} />
      }
      <div className="rounded-lg bg-green-50 p-3 text-xs text-green-800">
        {isCompare
          ? <><b>비교 모드:</b> 빨간 버블=BAU, 초록 버블=NetZero. 버블 위 마우스로 두 값을 동시에 확인합니다.</>
          : <><b>버블 크기</b>는 전체 연도 기준 고정 스케일 적용 — 연도 간 크기 비교 가능합니다.</>
        }
      </div>
    </>
  );
