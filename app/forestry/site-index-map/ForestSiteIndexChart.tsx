...
export function ForestSiteIndexChart({ stockRows, areaRows }: Props) {
  ...
  if (!isHistorical && mode === 'compare') {
    ...
    chartContent = (
      <>
        ...
        <div className="rounded-lg bg-green-50 p-3 text-xs text-green-800">
          <b>산림 탄소밀도:</b> 단위 면적당 탄소저장량 (t C/ha).
        </div>
      </>
    );
  }
  ...
  const gisTitle = isCompare
    ? `산림 탄소밀도 —${selectedYear}년 BAU(빨강) vs NetZero(초록) (t C/ha)`
    : `산림 탄소밀도 —${selectedYear}년 ${gisSc === 'Historical' ? '실적' : gisSc} (t C/ha)`;

  const gisContent = (
    <>
      <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800">
        📊 <b>산림 탄소밀도 = 탄소저장량 ÷ 산림면적 (t C/ha)</b>
      </div>
      <div className="flex flex-wrap items-center gap-4">{yearSelector}{!isHistorical && modeSelector}</div>
      {isCompare
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs text-blue-800">
        <p className="font-semibold">📊 산림 탄소밀도 = 탄소저장량 ÷ 산림면적 (t C/ha)</p>
        <p className="mt-1">권역별 단위면적당 탄소저장량으로, 산림의 탄소 집적 수준을 나타냅니다.</p>
      </div>
      {tabBar}
